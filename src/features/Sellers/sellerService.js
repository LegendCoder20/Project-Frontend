...
const getProducts = async (token) => {
  ...
  const response = await axios.get(
    "http://localhost:5000/api/sellers/product",
    config
  );
...
const getProduct = async (id, token) => {
  ...
  const response = await axios.get(
    `http://localhost:5000/api/sellers/product/${id}`,
    config
  );
...
const createProducts = async (productData, token) => {
  ...
  const response = await axios.post(
    "http://localhost:5000/api/sellers/product/add",
    productData,
    config
  );
...
const updateProduct = async (productData, id, token) => {
  console.log(productData, "Update Product from sellerService.js");
  ...
  const response = await axios.put(
    `http://localhost:5000/api/sellers/product/update/${id}`,
    productData,
    config
  );
